...
if modo == "📝 Registrar una idea":
    ...
    with st.form("solicitud_lab", clear_on_submit=True):
        ...
        c3, c4 = st.columns(2)
        empresa = c3.selectbox("Empresa / mundo", ["Grupo PDC", "PDC Brands", "Vikingo", "Nexo"])
        division_area = c4.text_input("División / Área")
        pais = st.selectbox("País", ["GT", "SV", "HN", "NI", "PN", "RD"])
        ...
        enviado = st.form_submit_button("Enviar solicitud", type="primary", use_container_width=True)
        ...
        if not solicitudes:
            ...
            for s in solicitudes:
                with st.expander(
                    ...
                    export_payload = {
                        "nombre": (s.nombre_app_propuesto or "").strip().lower().replace(" ", "-"),
                        "nombre_display": (s.nombre_app_propuesto or "").strip().title(),
                        "descripcion": s.descripcion_problema or "",
                        "area_pais": f"{s.division_area or ''} / {s.pais or ''}".strip(" /"),
                        "owner_email": s.correo_corporativo or "",
                    ...
                    st.download_button(
                        ...
                        file_name=f"{export_payload['nombre'] or 'solicitud'}-registro.json",
